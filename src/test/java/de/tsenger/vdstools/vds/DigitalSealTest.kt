...
import de.tsenger.vdstools.vds.DigitalSeal.Companion.fromByteArray
import de.tsenger.vdstools.vds.DigitalSeal.Companion.fromRawString
...
class DigitalSealTest {
    ...
    fun testParseSocialInsurranceCard() {
        val seal = fromByteArray(VdsRawBytes.socialInsurance)
    ...
    fun testParseArrivalAttestationV02() {
        val seal = fromByteArray(VdsRawBytes.arrivalAttestationV02)
        Assert.assertEquals(
    ...
    fun testParseResidentPermit() {
        val seal = fromByteArray(VdsRawBytes.residentPermit)
        Assert.assertEquals(
    ...
    fun testParseSupplementSheet() {
        val seal = fromByteArray(VdsRawBytes.supplementSheet)
        Assert.assertEquals(
            "ATD<<RESIDORCE<<ROLAND<<<<<<<<<<<<<<\n6525845096USA7008038M2201018<<<<<<06",
            seal!!.getFeature("MRZ")!!.valueStr()
        )
    ...
    fun testEmergencyTravelDoc() {
        val seal = fromByteArray(VdsRawBytes.emergenyTravelDoc)
        Assert.assertEquals(
            "I<GBRSUPAMANN<<MARY<<<<<<<<<<<<<<<<<\n6525845096USA7008038M2201018<<<<<<06",
            seal!!.getFeature("MRZ")!!.valueStr()
        )
    ...
    fun testParseAddressStickerId() {
        val seal = fromByteArray(VdsRawBytes.addressStickerId)
    ...
    fun testParseAddressStickerPassport() {
        val seal = fromByteArray(VdsRawBytes.addressStickerPassport)
    ...
    @Throws(IOException::class)
    fun testParseVisa() {
        val seal = fromByteArray(VdsRawBytes.visa_224bitSig)
        Assert.assertEquals(
    ...
    fun testParseFictionCert() {
        val seal = fromByteArray(VdsRawBytes.fictionCert)
        Assert.assertEquals(
    ...
    fun testParseTempPerso() {
        val seal = fromByteArray(VdsRawBytes.tempPerso)
        Assert.assertEquals(
    ...
    fun testParseTempPassport() {
        val seal = fromByteArray(VdsRawBytes.tempPassport)
        Assert.assertEquals(
    ...
    fun testGetEncodedBytes_aa() {
        val seal = fromByteArray(VdsRawBytes.arrivalAttestation)
    ...
    fun testgetRawString1() {
        ...
        val seal2 = fromRawString(rawString)
    ...
    fun testUnknowSealType() {
        val rawBytes = VdsRawBytes.permantResidencePermit
        rawBytes[16] = 0x99.toByte()
        val seal = fromByteArray(rawBytes)
