...
class SealCommonTest {
    ...
    fun testFromStringIdb() {
        assertTrue(parser.parse(IcbRawStringsCommon.TemporaryPassport) is IdbSeal)
    }

    @Test
    fun testFromStringIdb_fail() {
        assertFalse(parser.parse(IcbRawStringsCommon.TemporaryPassport) is VdsSeal)
    }


    @Test
    fun testIdbGetMessageTag() {
        val seal = parser.parse(IcbRawStringsCommon.TemporaryPassport)
    ...
    @Test
    fun testIdbGetMessageTag_newApi() {
        val seal = parser.parse(IcbRawStringsCommon.TemporaryPassport)
        val mrz = seal.getMessageByTag(8)?.value.toString()
        assertEquals(
            "PPD<<FOLKS<<TALLULAH<<<<<<<<<<<<<<<<<<<<<<<<\n3113883489D<<9709155F1601013<<<<<<<<<<<<<<04", mrz
    ...
    @Test
    fun testIdbGetMessageName_newApi() {
        val seal = parser.parse(IcbRawStringsCommon.TemporaryPassport)
        val mrz = seal.getMessageByName("MRZ_TD3")?.value.toString()
        assertEquals(
            "PPD<<FOLKS<<TALLULAH<<<<<<<<<<<<<<<<<<<<<<<<\n3113883489D<<9709155F1601013<<<<<<<<<<<<<<04", mrz
    ...
    fun testFromStringVds() {
        ...
        assertTrue(parser.parse(rawString) is VdsSeal)
    }
    ...
    @Test
    fun testVdsGetMessageTag_newApi() {
        val rawString = DataEncoder.encodeBase256(VdsRawBytesCommon.emergenyTravelDoc)
        val seal = parser.parse(rawString)
        val mrz = seal.getMessageByTag(2)?.value.toString()
        assertEquals(
    ...
    @Test
    fun testVdsGetMessageName_newApi() {
        val rawString = DataEncoder.encodeBase256(VdsRawBytesCommon.emergenyTravelDoc)
        val seal = parser.parse(rawString)
        val mrz = seal.getMessageByName("MRZ")?.value.toString()
        assertEquals(
    ...
    fun testBaseDokumentTypeNullIdb() {
        val seal = parser.parse(IcbRawStringsCommon.ProvisionalResidenceDocument)
        assertNotNull(seal)
