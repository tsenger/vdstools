...
import vdstools.vds.dto.FeaturesDto
import vdstools.vds.dto.SealDto
...
class FeatureConverter @JvmOverloads constructor(inputStream: InputStream? = null) {
    ...
    private fun <T> decodeFeature(sealDto: SealDto, derTlv: DerTlv): T {
        ...
        return when (coding) {
            FeatureCoding.C40 -> decodeC40Feature(sealDto, derTlv) as T
            FeatureCoding.UTF8_STRING -> String(derTlv.value, StandardCharsets.UTF_8) as T
            FeatureCoding.BYTE -> derTlv.value[0] as T
            FeatureCoding.BYTES -> derTlv.value as T
            else -> derTlv.value as T
        }
    }
